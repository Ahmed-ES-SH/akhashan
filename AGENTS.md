...
- `app/helpers` -`{componentName}` -`{featureName}`

- `app/hooks` -`{componentName}` -`{hookfeatureName}`

- `app/_components` -`website/{_pageName}` or `dashboard/{_pageName}` -`{component}`

- `app/types` -`website/{pageName.types.ts}` or `dashboard/{pageName.types.ts}`
