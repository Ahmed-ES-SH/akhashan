...
**Approach**: Step-by-step, one file/component at a time
...
**Files**: `AdminServiceForm.tsx`, `AdminServiceRow.tsx`, `AdminServicesManager.tsx`, `AdminLoginForm.tsx`
...
- Verify RTL layout for Arabic locale (search icon position, sort dropdown direction)

---

## Execution Order

1. **Step 1** — IconPicker integration (highest impact, user specifically requested)
2. **Step 2** — Filter UI (user requested, depends on Step 1 being done)
3. **Step 3** — Remove drag handle + fix row actions (quick win, P1)
4. **Step 4** — Color contrast fixes (quick win, P2)
5. **Step 5A** — Safe token migration (colors, borders, text, backgrounds)
6. **Step 5B** — Radius and shadow normalization (visual hierarchy)
7. **Step 6** — Toggle loading state (P1, depends on Step 5A)
8. **Step 7** — Accessibility improvements (focus management, pagination aria)
9. **Step 8** — Translate hardcoded strings (i18n, minor)
10. **Step 9** — Lint + verify (final pass)
...
| File | Steps | Changes |
|------|-------|---------|
| `app/_components/website/_admin/AdminServiceForm.tsx` | 1, 4, 5A, 5B, 7 | IconPicker integration, color fixes, token migration, focus management |
| `app/_components/website/_admin/AdminServiceRow.tsx` | 3, 4, 5A, 5B, 6, 7 | Remove drag handle, always-visible actions, color fixes, token migration, toggle loading, focus management |
| `app/_components/website/_admin/AdminServicesManager.tsx` | 2, 5A, 8 | Search + sort UI, debounced filtering, token migration, translation updates |
| `app/_components/website/_admin/AdminLoginForm.tsx` | 5A | Already uses tokens — verify consistency |
| `app/_components/IconPicker.tsx` | 1 | Controlled API, design token styling, memoization |
| `app/_components/website/_admin/AdminPagination.tsx` | 7 | Add `aria-current="page"` |
| `translations/en.json` | 2, 8 | Add filter, action, dialog, picker translation keys |
| `translations/ar.json` | 2, 8 | Add Arabic equivalents |
